...
class PongConsumer(AsyncWebsocketConsumer):
	...
	async def connect(self):
		...
		self.player_id = self.scope["id"]
		if self.player_id == -1: 
			print("Invalid token")
			await self.close()
		else:
			player = await database_sync_to_async(Player.objects.get_or_create)(pid=self.player_id)
			await self.accept()
			user_queue.add(self)
		...
		# self.scope["user"] = database_sync_to_async(Game.objects.get_or_create)(host=self.scope["id"])
	...
	async def create_room(self):
		...
		# try:
		# 	# Fetch the game in an async-safe manner
		# 	game = await database_sync_to_async(Game.objects.get)()
		# except Game.DoesNotExist:
		p1 = await database_sync_to_async(Player.objects.get)(pid=self.player_id)
		p2 = await database_sync_to_async(Player.objects.get)(pid=player2.player_id)
		game = await database_sync_to_async(Game.objects.create)(
			p1=p1,
			p2=p2,
		)
		self.game_state[self.room_name]['gid'] = game.gid
	...
	async def game_loop(self):
		while True:
			...
			if self.game_state[self.room_name]["players"]["paddle1"]["score"] == 5 or self.game_state[self.room_name]["players"]["paddle2"]["score"] == 5:
				break
		game = await database_sync_to_async(Game.objects.get)(gid=self.game_state[self.room_name]['gid'])
		...
		winner = database_sync_to_async(Player.objects.get)(pid=game.winner)
		winner.wins += 1
		await database_sync_to_async(winner.save)()
		loser = database_sync_to_async(Player.objects.get)(pid=game.loser)
		loser.losses += 1
	...
	async def disconnect(self, close_code):
		...
		if hasattr(self, 'room_name') and self.room_name in self.game_state:
			if self.player_id == self.game_state[self.room_name]["players"]["paddle1"]["id"]:
				...
				pid = self.game_state[self.room_name]["players"]["paddle2"]["id"]
				...
				pid = self.game_state[self.room_name]["players"]["paddle1"]["id"]
